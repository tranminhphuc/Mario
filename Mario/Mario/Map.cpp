...
{
	listBlock[16]->Update(time);
	listBlock[18]->Update(time);
	listBlock[73]->Update(time);
	listBlock[74]->Update(time);
	listBlock[75]->Update(time);
}
...
void Map::DrawMap(RenderWindow & window)
{
	for (int i = GetStartMap(), iEnd = GetEndMap(); i < iEnd && i < mapWidth; i++)
	{
		...
		{
			if (listObject[i][j]->GetID() != 0)
			{
				listBlock[listObject[i][j]->GetID()]->Draw(window, (float)(32 * i + Xmap), (float)(j * 32));
			}
		}
	}
}
...
void Map::LoadGameData()
{
	...
	time.push_back(0);
	listBlock.push_back(new Animation(name, time));
	name.clear();
	time.clear();
	// ----- 1 -----
...
void Map::DrawGND(int x, int y, int width, int height)
{
	for (int i = 0; i < width; i++)
	{
		...
		{
			listObject[x + i][y - j]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 1 : maptype == LongDat ? 2 : maptype == Bien ? 3 : maptype == DiaNguc ? 4 : maptype == Ho ? 6 : 5);
		}
	}
}
...
void Map::DrawGND2(int x, int y, int width, int height)
{
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			listObject[x + i][y - j]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 13 : maptype == NuiLua ? 15 : 14);
		}
	}
}
...
void Map::DrawGND2(int x, int y, int size, bool direction)
{
	...
	else
	{
		for (int i = 0, k = size; i < size; i++)
		{
			for (int j = k; j > 0; j--)
			{
				listObject[x + i][y - j + 1]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 13 : maptype == NuiLua ? 15 : 14);
			}
			k--;
		}
	}
...
void Map::DrawBrick(int x, int y, int width, int height)
{
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			listObject[x + i][y - j]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 10 : maptype == LongDat ? 11 : 12);
		}
	}
}
...
void Map::DrawBlockQ(int x, int y, int width)
{
	for (int i = 0; i < width; i++)
	{
		listObject[x + i][y]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 16 : 17);
	}
}
...
void Map::DrawPiPe(int x, int y, int height)
{
	...
	{
		listObject[x][y - i]->SetID(maptype == NgoaiTroiBanNgay ? 21 : maptype == Bien ? 41 : maptype == NgoaiTroiBanDem ? 51 : maptype == ThienDuong ? 61 : maptype == NuiLua ? 69 : maptype == Ho ? 65 : 31);
		listObject[x + 1][y - i]->SetID(maptype == NgoaiTroiBanNgay ? 22 : maptype == Bien ? 42 : maptype == NgoaiTroiBanDem ? 52 : maptype == ThienDuong ? 62 : maptype == NuiLua ? 70 : maptype == Ho ? 66 : 32);
	}

	listObject[x][y - height]->SetID(maptype == NgoaiTroiBanNgay ? 23 : maptype == Bien ? 43 : maptype == NgoaiTroiBanDem ? 53 : maptype == ThienDuong ? 63 : maptype == NuiLua ? 71 : maptype == Ho ? 67 : 33);
	listObject[x + 1][y - height]->SetID(maptype == NgoaiTroiBanNgay ? 24 : maptype == Bien ? 44 : maptype == NgoaiTroiBanDem ? 54 : maptype == ThienDuong ? 64 : maptype == NuiLua ? 72 : maptype == Ho ? 68 : 34);
}

void Map::DrawPiPeHorizontal(int x, int y, int width)
{
	listObject[x][y]->SetID(maptype == NgoaiTroiBanNgay ? 25 : maptype == Bien ? 45 : maptype == NgoaiTroiBanDem ? 55 : 35);
	listObject[x][y - 1]->SetID(maptype == NgoaiTroiBanNgay ? 26 : maptype == Bien ? 46 : maptype == NgoaiTroiBanDem ? 56 : 36);

	for (int i = 0; i < width; i++)
	{
		listObject[x + i + 1][y]->SetID(maptype == NgoaiTroiBanNgay ? 27 : maptype == Bien ? 47 : maptype == NgoaiTroiBanDem ? 57 : 37);
		listObject[x + i + 1][y - 1]->SetID(maptype == NgoaiTroiBanNgay ? 28 : maptype == Bien ? 48 : maptype == NgoaiTroiBanDem ? 58 : 38);
	}

	listObject[x + width + 1][y]->SetID(maptype == NgoaiTroiBanNgay ? 29 : maptype == Bien ? 49 : maptype == NgoaiTroiBanDem ? 59 : 39);
	listObject[x + width + 1][y - 1]->SetID(maptype == NgoaiTroiBanNgay ? 30 : maptype == Bien ? 50 : maptype == NgoaiTroiBanDem ? 60 : 40);
}
...
void Map::DrawPiPeVertical(int x, int y, int height)
{
	for (int i = 0; i < height; i++)
	{
		listObject[x][y - i]->SetID(maptype == NgoaiTroiBanNgay ? 21 : maptype == Bien ? 41 : maptype == NgoaiTroiBanDem ? 51 : 31);
		listObject[x + 1][y - i]->SetID(maptype == NgoaiTroiBanNgay ? 22 : maptype == Bien ? 42 : maptype == NgoaiTroiBanDem ? 52 : 32);
	}
}
...
void Map::DrawCoin(int x, int y, int width, int height)
{
	for (int i = 0; i < width; i++)
	{
		for (int j = 0; j < height; j++)
		{
			listObject[x + i][y - j]->SetID(maptype == NgoaiTroiBanNgay || maptype == NgoaiTroiBanDem ? 73 : maptype == Bien ? 75 : 74);
		}
	}
}
...
void Map::DrawEnd(int x, int y, int height)
{
	for (int i = 0; i < height; i++)
	{
		listObject[x][y - i]->SetID(maptype == NgoaiTroiBanDem ? 78 : 76);
	}
	listObject[x][y - height]->SetID(maptype == NgoaiTroiBanDem ? 79 : 77);
}
...
void Map::DrawCastleSmall(int x, int y)
{
	for (int i = 0; i < 2; i++)
	{
		listObject[x][y - i]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 1][y - i]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 2][y - i]->SetID(maptype == NuiLua ? 88 : 81);
		listObject[x + 3][y - i]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 4][y - i]->SetID(maptype == NuiLua ? 87 : 80);
	}

	listObject[x + 2][y - 1]->SetID(maptype == NuiLua ? 89 : 82);

	for (int i = 0; i < 5; i++)
	{
		listObject[x + i][y - 2]->SetID(i == 0 || i == 4 ? maptype == NuiLua ? 93 : 86 : maptype == NuiLua ? 92 : 85);
	}

	listObject[x + 1][y - 3]->SetID(maptype == NuiLua ? 90 : 83);
	listObject[x + 2][y - 3]->SetID(maptype == NuiLua ? 87 : 80);
	listObject[x + 3][y - 3]->SetID(maptype == NuiLua ? 91 : 84);

	for (int i = 0; i < 3; i++)
	{
		listObject[x + i + 1][y - 4]->SetID(maptype == NuiLua ? 93 : 86);
	}
}
...
void Map::DrawCastleBig(int x, int y)
{
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			listObject[x + i][y - j]->SetID(maptype == NuiLua ? 87 : 80);
			listObject[x + i + 7][y - j]->SetID(maptype == NuiLua ? 87 : 80);
		}
	}

	for (int i = 0; i < 3; i++)
	{
		listObject[x + 2 + i * 2][y]->SetID(maptype == NuiLua ? 88 : 81);
		listObject[x + 2 + i * 2][y - 1]->SetID(maptype == NuiLua ? 89 : 82);
	}

	for (int i = 0; i < 9; i++)
	{
		listObject[x + i][y - 2]->SetID(maptype == NuiLua ? 87 : 80);
	}
	...
	for (int i = 0; i < 9; i++)
	{
		if (i < 2 || i > 6)
		{
			listObject[x + i][y - 5]->SetID(maptype == NuiLua ? 93 : 86);
		}
		else
		{
			listObject[x + i][y - 5]->SetID(maptype == NuiLua ? 92 : 85);
		}
	}

	for (int i = 0; i < 2; i++)
	{
		listObject[x + 3 + i * 2][y]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 3 + i * 2][y - 1]->SetID(maptype == NuiLua ? 87 : 80);
	}

	for (int i = 0; i < 2; i++)
	{
		listObject[x + 3 + i * 2][y - 3]->SetID(maptype == NuiLua ? 88 : 81);
		listObject[x + 3 + i * 2][y - 4]->SetID(maptype == NuiLua ? 89 : 82);
	}

	for (int i = 0; i < 3; i++)
	{
		listObject[x + 2 + i * 2][y - 3]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 2 + i * 2][y - 4]->SetID(maptype == NuiLua ? 87 : 80);
	}

	for (int i = 0; i < 2; i++)
	{
		listObject[x + 2][y - 6 - 1]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 3][y - 6 - 1]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 5][y - 6 - 1]->SetID(maptype == NuiLua ? 87 : 80);
		listObject[x + 6][y - 6 - 1]->SetID(maptype == NuiLua ? 87 : 80);
	}

	listObject[x + 4][y - 6]->SetID(maptype == NuiLua ? 88 : 81);
	listObject[x + 4][y - 7]->SetID(maptype == NuiLua ? 89 : 82);

	for (int i = 0; i < 3; i++)
	{
		listObject[x + 3 + i][y - 8]->SetID(maptype == NuiLua ? 92 : 85);
	}

	listObject[x + 2][y - 8]->SetID(maptype == NuiLua ? 93 : 86);
	listObject[x + 6][y - 8]->SetID(maptype == NuiLua ? 93 : 86);

	listObject[x + 3][y - 9]->SetID(maptype == NuiLua ? 90 : 83);
	listObject[x + 4][y - 9]->SetID(maptype == NuiLua ? 87 : 80);
	listObject[x + 5][y - 9]->SetID(maptype == NuiLua ? 91 : 84);

	for (int i = 0; i < 3; i++)
	{
		listObject[x + 3 + i][y - 10]->SetID(maptype == NuiLua ? 93 : 86);
	}
}
...
void Map::DrawCastleWall(int x, int y, int width, int height)
{
	...
	for (int i = 0; i < width; i++)
	{
		listObject[x + i][y - height - 1]->SetID(maptype == NuiLua ? 89 : 82);
	}
}
